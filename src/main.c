...
//        risc_rect.h = clamp(h, 32, MAX_HEIGHT);
  ...
  //  risc_configure_memory(risc, mem_option, rtc_option, risc_rect.w, risc_rect.h, color_option);
    risc_configure_memory(risc, mem_option, rtc_option, MAX_WIDTH, MAX_HEIGHT, color_option);
  }
   ...
   for (int line = damage.y2; line >= damage.y1; line--) {
     ...
     for (int col = damage.x1; col <= damage.x2; col++) {
       ...
       if (color) {
         ...
         for (int b = 0; b < 32; b++) {
           // printf("%d,%d", col*32+b, line);
           rfbDrawPixel(screen, col*32+b, MAX_HEIGHT - line - 1, (pixels & 1) ? WHITE : BLACK);
           pixels >>= 1;
         }
       }
     }
